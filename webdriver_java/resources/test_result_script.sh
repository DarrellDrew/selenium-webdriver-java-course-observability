...
TEST_RESULTS_LOCATION="${1:-/home/runner/work/selenium-webdriver-java-course-observability/selenium-webdriver-java-course-observability/webdriver_java/target/surefire-reports}"
...
gibhub_actions_total_tests{action_id="${GITHUB_RUN_NUMBER}",commit="${GITHUB_SHA}", actor="${GITHUB_ACTOR}", branch="${GITHUB_REF}"} $(echo ${TEST_RESULTS_STRING} | awk -F'"' '{ print $4 }')
